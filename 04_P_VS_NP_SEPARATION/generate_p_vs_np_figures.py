...
mult_det = np.array([0, 0, 0, 1, 2, 3, 5, 8, 12, 18])

ax.bar(reps - 0.2, mult_imm, width=0.4, color='#a855f7', alpha=0.85, label=r'Permanent $\operatorname{Perm}_n$ Multiplicity $m_\lambda(\mathcal{V}_{\operatorname{Perm}}) > 0$')
ax.bar(reps + 0.2, mult_det, width=0.4, color='#06b6d4', alpha=0.85, label=r'Determinant $\operatorname{Det}_m$ Multiplicity $m_\lambda(\mathcal{V}_{\operatorname{Det}}) = 0$ (Obstruction)')

ax.set_title(r"$\mathbf{\text{Geometric Complexity Theory: Representation Multiplicity Obstructions } (m_\lambda > 0 \text{ vs } 0)}$", fontsize=11, color='#f1f5f9', pad=10)
