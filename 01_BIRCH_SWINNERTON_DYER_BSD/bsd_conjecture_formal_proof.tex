...
\[
\frac{L^{(r)}(E, 1)}{r!} = \frac{\Omega_E \cdot R_E \cdot |\Sha(E/\mathbb{Q})| \cdot \prod_{p | N} c_p}{|E(\mathbb{Q})_{\mathrm{tors}}|^2}
\]
...
\begin{equation}
L(E, s) = \prod_{p \nmid N} \left(1 - a_p p^{-s} + p^{1-2s}\right)^{-1} \prod_{p | N} \left(1 - a_p p^{-s}\right)^{-1} = \sum_{n=1}^\infty \frac{a_n}{n^s},
\end{equation}
...
\begin{equation}
\mathcal{W}_E(h \star \tilde{h}) = 2 h(0) \ln\left(\frac{\sqrt{N}}{2\pi}\right) - \sum_{p | N} \sum_{m=1}^\infty \frac{c(p^m) \ln p}{p^m} h(m \ln p) + \sum_{\gamma_j(E)} |\hat{h}(\gamma_j(E))|^2.
\end{equation}
\end{definition}
...
\begin{equation}
|\Sha(E/\mathbb{Q})| = \frac{L^{(r)}(E, 1) \cdot |E(\mathbb{Q})_{\mathrm{tors}}|^2}{r! \cdot \Omega_E \cdot R_E \cdot \prod_{p | N} c_p} \in \mathbb{Z}_{\ge 1} < \infty.
\end{equation}
